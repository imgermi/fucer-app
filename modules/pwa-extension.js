...
export default function nuxtWorkboxExtension() {
  ...
  this.nuxt.hook("build:before", async (nuxt) => {
    ...
    cachingExtensions = Array.isArray(cachingExtensions)
      ? cachingExtensions
      : [cachingExtensions];
    nuxt.options.pwa.workbox.cachingExtensions = cachingExtensions.concat([
      "~/modules/pwa-extension-inject.js",
    ]);
  });
  this.nuxt.hook("build:compiled", async () => {
    const swFile = path.resolve(
      this.options.srcDir,
      this.options.dir.static || "static",
      "sw.js"
    );
